def divide(quotients, divisors):
    ...
    """
    res=[[] for i in range(len(quotients))]
    for i in range(len(divisors)):
        for j in (range(len(quotients))):
            if divisors[i]%quotients[j]==0:
                res[j].append(divisors[i])
    return {quotients[i]: res[i] for i in range(len(quotients))}
    ...
    def add(fruits, amount, cart):
        if fruits == [] and amount == 0:
            ...
            for k in range(1,amount):
                ...
                add(fruits[1:len(fruits)],amount-k*price,cart+"["+ "%d" % k +" "+fruit+"]")
                fruit=fruits[0]
