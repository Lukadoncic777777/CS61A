...
def balanced(m):
    ...
    if is_planet(m):
        return True
    if total_mass(end(right(m)))*length(right(m))==total_mass(end(left(m)))*length(left(m)) and balanced(end(left(m))) and balanced(end(right(m))):
        return True
    return False
