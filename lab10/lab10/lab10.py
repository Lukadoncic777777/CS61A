...
def calc_eval(exp):
    ...
    if isinstance(exp, Pair):
        ...
        if operator == 'and': # and expressions
            ...
            return calc_apply(eval_define(operator),operands)
    elif exp in OPERATORS:   # Looking up procedures
        return OPERATORS[exp]
    elif isinstance(exp, int) or isinstance(exp, bool):   # Numbers and booleans
        return exp
    elif isinstance(exp,str): # CHANGE THIS CONDITION FOR Q4 where are variables stored?
        return eval_define(exp) # UPDATE THIS FOR Q4, how do you access a variable?
...
def eval_and(expressions):
    ...
    def fun(exp):
        ...
        if isinstance(exp,bool) or isinstance(exp,int):
            return exp
        if isinstance(exp.first,str):
            if exp.first in ['define','+','-','*','/','//']:
                return calc_eval(exp)
            if exp.first=='and':
                return eval_and(exp.rest)
        # if fun(exp.first)==False:
        #     return False
        Fi=fun(exp.first)
        if Fi is scheme_f:
            return False
